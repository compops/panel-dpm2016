...
gibbs_finitemixture <- function(y, x, nIter, nMaxClusters, prior, postGrid)  {
  ...
  post_alpha        <- matrix(0, nrow=nIter+1, ncol=1)
  ...
  post_beta         <- matrix(0, nrow=nIter+1, ncol=nMaxClusters)
  post_Qinv         <- matrix(1, nrow=nIter+1, ncol=nMaxClusters)
  post_betaInd      <- matrix(0, nrow=nIter+1, ncol=nIndividuals)
  ...
  post_betaInd[1,]  <- rnorm(nIndividuals, a0star[2], sqrt(A0star[2,2]))
  ...
  for (kk in 1:nIter) {
    ...
    for ( ii in 1:nIndividuals ) {
      ...
      V  <- x[,ii,] %*% t(x[,ii,]) / post_Qinv[kk,I[kk,ii]] + post_s2e[kk] / post_omega[ kk, ii ] * diag(nObservations)
    ...
    post_alpha[kk+1]    <- alphastar[1]
    post_beta[kk+1,]    <- matrix(alphastar[-1],nrow=nMaxClusters,byrow = TRUE)
    ...
    for (jj in 1:nMaxClusters) {
      ...
      if ( nMembers[kk,jj] > 1 ) {
        ...
        post_Qinv[kk+1,jj] <- rgamma(1, shape=cQ, rate=CQ)
      } else {
        
        # Sampling Q_k from Wishart prior or Gamma prior
        #Qinv[jj] <- rWishart(1, c0Q, C0Q)
        post_Qinv[kk+1,jj] <- rgamma(1, shape=c0Q, rate=C0Q)
      }
      
      # Compute p( beta_i^s | y ) via (8.48) on page 264
      post_randomeffects[kk,] = post_randomeffects[kk,] + post_eta[kk+1,jj] * dnorm( postGrid, post_beta[kk+1,jj],sqrt(1/post_Qinv[kk+1,jj]))
    }
    ...
    for ( ii in 1:nIndividuals ) {
      z     <- y[ii,] - post_alpha[kk+1]  - post_betaInd[kk,ii] * x[,ii,]
    ...
    for (ii in 1:nIndividuals) {
      for (jj in 1:nMaxClusters) {
        ...
        V  <- x[,ii,] %*% t(x[,ii,]) / post_Qinv[kk+1,jj] + post_s2e[kk+1] / post_omega[ kk, ii ] * diag(nObservations)
        
        # Compute the weigth from (8.55)
        pi[ii,jj] <- log(post_eta[kk+1,jj]) + dmvnorm( y[ii,], post_alpha[kk+1] - post_beta[kk+1,jj] * x[,ii,], V, log=TRUE )
      }
    ...
    for (ii in 1:nIndividuals) {
      ...
      bS <- solve( post_Qinv[kk+1,I[kk+1,ii]] + t(x[,ii,]) %*% x[,ii,] * post_omega[kk,ii] / post_s2e[kk+1] )
      
      # Compute the posterior mean using (8.52)
      bs <- bS %*% ( post_Qinv[kk+1,I[kk+1,ii]] %*% post_beta[kk,I[kk+1,ii]] + x[,ii,] %*% (y[ii,] - post_alpha[kk+1] ) * post_omega[kk,ii] / post_s2e[kk+1] )
    ...
    for (ii in 1:nIndividuals) {
      ...
      z  <- y[ii,] - post_alpha[kk+1] - post_betaInd[kk+1,ii] * x[,ii,]
  ...
  list(alpha=post_alpha, var=sweep(1/post_omega,1,post_s2e,'*'), eta=post_eta, 
       ...
       posterior_beta=post_randomeffects, betaInd=post_betaInd, beta=post_beta,
       Qinv=post_Qinv)
}
...
gibbs_dpm <- function(y, x, nIter, nMaxClusters, prior, postGrid)  {
  ...
  list(alpha=post_alpha, var=sweep(1/post_omega,1,post_s2e,'*'), eta=post_eta, 
       nOccupiedClusters=nOccupiedClusters, nMembers=nMembers,
       posterior_beta=post_randomeffects, betaInd=post_betaInd, beta=post_beta,
       Qinv=post_Qinv, alphaSB=post_alphaSB)  
